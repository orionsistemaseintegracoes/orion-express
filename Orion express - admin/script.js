...
/* ================= FILIAIS (MVP MULTIUNIDADE) ================= */
const FILIAL_PADRAO_ID = 'matriz';
let filialAtivaId = localStorage.getItem('orion_filial_ativa') || FILIAL_PADRAO_ID;
let filiaisLista = [];
let perfilOperadorAtual = null;
const dadosConsolidadosFiliais = {};

function idFilialAtual() {
  return filialAtivaId || FILIAL_PADRAO_ID;
}
...
function comFilial(dados = {}) {
  return { ...dados, filialId: idFilialAtual() };
}

function idConfigFilial(chave) {
  return idFilialAtual() === FILIAL_PADRAO_ID ? chave : `${idFilialAtual()}__${chave}`;
}

function refConfigFilial(chave) {
  return dbFirestore.collection('config').doc(idConfigFilial(chave));
}

function documentosComFilial(snapshot, colecao) {
  const todos = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
  dadosConsolidadosFiliais[colecao] = todos;
  return todos.filter(item => filialDoRegistro(item) === idFilialAtual());
}

function catalogoFiliais() {
  const itens = filiaisLista.slice();
  if (!itens.some(f => f.id === FILIAL_PADRAO_ID)) itens.unshift(filialPorId(FILIAL_PADRAO_ID));
  return itens;
}

function consultaFilial(nome) {
  let consulta = dbFirestore.collection(nome);
  if (perfilOperadorAtual && perfilOperadorAtual.role !== 'admin') {
    consulta = consulta.where('filialId', '==', idFilialAtual());
  }
  return consulta;
}

function filiaisPermitidasDoUsuario(usuario = perfilOperadorAtual) {
  if (!usuario) return [FILIAL_PADRAO_ID];
  if (usuario.role === 'admin') return catalogoFiliais().map(f => f.id);
  return Array.isArray(usuario.filiais) && usuario.filiais.length
    ? usuario.filiais
    : [FILIAL_PADRAO_ID];
}

function usuarioPodeAcessarFilial(filialId) {
  return filiaisPermitidasDoUsuario().includes(filialId || FILIAL_PADRAO_ID);
}

function filialPorId(id) {
  return filiaisLista.find(f => f.id === id) || {
    id: FILIAL_PADRAO_ID,
    nome: 'Filial Matriz',
    codigo: 'MATRIZ',
    ativa: true
...
function renderizarSeletorFilial() {
  const select = document.getElementById('filial-ativa-select');
  const badge = document.getElementById('filial-ativa-label');
  const permitidas = filiaisPermitidasDoUsuario();
  const disponiveis = catalogoFiliais()
    .filter(f => f.ativa !== false && permitidas.includes(f.id));
  if (!disponiveis.some(f => f.id === filialAtivaId)) {
    filialAtivaId = disponiveis[0]?.id || FILIAL_PADRAO_ID;
    localStorage.setItem('orion_filial_ativa', filialAtivaId);
  }
  if (select) {
    select.innerHTML = disponiveis.map(f => `<option value="${escapeDashboard(f.id)}">${escapeDashboard(f.nome || f.codigo || f.id)}</option>`).join('');
    select.value = filialAtivaId;
    select.disabled = disponiveis.length <= 1;
  }
  if (badge) badge.textContent = filialPorId(filialAtivaId).nome || filialAtivaId;
}

function selecionarFilialAtiva(filialId) {
  if (!usuarioPodeAcessarFilial(filialId)) {
    return showToast('Você não possui acesso a esta filial.', 'error');
  }
  filialAtivaId = filialId || FILIAL_PADRAO_ID;
  localStorage.setItem('orion_filial_ativa', filialAtivaId);
...
function abrirModalConfirmacao({ titulo, mensagem, detalhe, textoBtn, corBtn, icone, iconeCor, onConfirm }) {
  ...
  if (!modal || !elTitulo || !elMensagem || !elBtn || !elIcone) return;
  ...
  elIcone.className = `mx-auto mb-4 w-14 h-14 rounded-full flex items-center justify-center ${iconeCor || 'bg-red-500/20'}`;
  elIcone.innerHTML = `<i class="fa ${icone || 'fa-exclamation-triangle'} text-2xl ${iconeCor ? 'text-white' : 'text-red-400'}"></i>`;
  _confirmacaoCallback = onConfirm || null;
  elBtn.onclick = () => { const cb = _confirmacaoCallback; fecharModalConfirmacao(); if (cb) cb(); };
...
function iniciarSincronizacao() {
  dbFirestore.collection('filiais').orderBy('nome', 'asc').onSnapshot((snapshot) => {
    filiaisLista = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
    renderizarSeletorFilial();
    renderizarGestaoFiliais();
    renderizarBalancoFiliais();
  }, (error) => {
    console.warn('Coleção de filiais ainda não disponível; usando filial Matriz virtual.', error);
    filiaisLista = [filialPorId(FILIAL_PADRAO_ID)];
    renderizarSeletorFilial();
  });

  // Mesas: Atualiza lista e a grade
  consultaFilial('mesas').orderBy('numero', 'asc').onSnapshot((snapshot) => {
    mesasLista = documentosComFilial(snapshot, 'mesas');
    if (typeof renderizarMesas === 'function') renderizarMesas();
  });

  // Clientes: Atualiza lista e o SELECT de venda
  consultaFilial('clientes').onSnapshot((snapshot) => {
    db.clientes = documentosComFilial(snapshot, 'clientes');
    renderizarClientes();
  ...
  consultaFilial('estoque').onSnapshot((snapshot) => {
    db.estoque = documentosComFilial(snapshot, 'estoque');
    renderizarEstoque();
  ...
  consultaFilial('vendas').orderBy('dataIso', 'desc').onSnapshot((snapshot) => {
    db.vendas = documentosComFilial(snapshot, 'vendas');
  ...
  consultaFilial('caixas').orderBy('data_abertura', 'desc').onSnapshot((snapshot) => {
    caixasLista = documentosComFilial(snapshot, 'caixas');
  ...
  consultaFilial('movimentacoes_caixa').orderBy('data_hora', 'desc').onSnapshot((snapshot) => {
    movimentacoesCaixa = documentosComFilial(snapshot, 'movimentacoes_caixa');
  ...
  refConfigFilial('pagamentos').onSnapshot((doc) => {
    const data = doc.data();
  ...
  refConfigFilial('classificacoes').onSnapshot((doc) => {
    const data = doc.data() || {};
  ...
  consultaFilial('fornecedores').orderBy('nome').onSnapshot((snapshot) => {
    fornecedoresLista = documentosComFilial(snapshot, 'fornecedores');
  ...
  consultaFilial('entradas').orderBy('dataIso', 'desc').onSnapshot((snapshot) => {
    entradasLista = documentosComFilial(snapshot, 'entradas');
    renderizarEntradas();
  });

  // Contas a pagar (tela Contas a Pagar): contas e baixas.
  consultaFilial('contas_pagar').orderBy('dataIso', 'desc').onSnapshot((snapshot) => {
    contasPagarLista = documentosComFilial(snapshot, 'contas_pagar');
  ...
  consultaFilial('movimentacoes_estoque').orderBy('dataIso', 'desc').onSnapshot((snapshot) => {
    movimentacoesEstoque = documentosComFilial(snapshot, 'movimentacoes_estoque');
  });
  ...
  const atualizarUsuarios = usuarios => {
    ...
    perfilOperadorAtual = db.usuarios.find(u => u.id === euLogado) || perfilOperadorAtual;
    renderizarSeletorFilial();
  ...
  if (ehAdministrador()) {
    dbFirestore.collection('usuarios').onSnapshot(snapshot => {
      atualizarUsuarios(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })));
    });
  } else if (auth.currentUser) {
    dbFirestore.collection('usuarios').doc(auth.currentUser.uid).onSnapshot(doc => {
      atualizarUsuarios(doc.exists ? [{ id: doc.id, ...doc.data() }] : []);
    });
  }
  ...
  consultaFilial('logs')
    ...
    .onSnapshot((snapshot) => {
    db.logs = documentosComFilial(snapshot, 'logs');
...
async function proximoNumeroPedido() {
  ...
  const resultado = await dbFirestore.runTransaction(async (tx) => {
    ...
    tx.set(ref, { numero: proximo, filialId: idFilialAtual() }, { merge: true });
    return proximo;
  });
...
async function migrarNumeroPedido() {
  try {
    ...
    let max = 0;
    const reformatar = [];
    const docsFilial = snapshot.docs.filter(doc => filialDoRegistro(doc.data()) === idFilialAtual());
    docsFilial.forEach(doc => {
    ...
    if (max > atual) {
      await ref.set({ numero: max, filialId: idFilialAtual() }, { merge: true });
    }
    ...
    const pendentes = docsFilial.filter(doc => {
...
auth.onAuthStateChanged(async (user) => {
  if (user) {
    ...
    try {
      ...
      perfilOperadorAtual = doc.exists ? { id: doc.id, ...doc.data() } : null;
      ...
      if (!permitido && perfisEmCriacao.has(user.email)) {
        ...
        for (let i = 0; i < 20; i++) {
          ...
          if (novo.exists) {
            permitido = true;
            perfilOperadorAtual = { id: novo.id, ...novo.data() };
            break;
          }
        }
      }
    ...
    const acessosFilial = filiaisPermitidasDoUsuario(perfilOperadorAtual);
    if (!acessosFilial.includes(filialAtivaId)) {
      filialAtivaId = acessosFilial[0] || FILIAL_PADRAO_ID;
      localStorage.setItem('orion_filial_ativa', filialAtivaId);
    }
    ...
    if (ehAdministrador()) migrarNumeroPedido(); // Migração global é tarefa administrativa.
...
function mostrarPagina(id) {
  ...
  if (targetPage) {
    ...
    if (id === 'config') { carregarPerfilNoConfig(); renderizarControleAcesso(); renderizarGestaoFiliais(); } // Carrega dados do perfil + controle de acesso
    ...
    if (id === 'balanco-filiais') renderizarBalancoFiliais();
...
async function loadBannerConfig() {
  try {
    const lojaDoc = await refConfigFilial('loja').get();
    if (lojaDoc.exists) {
    ...
    const bannerDoc = await refConfigFilial('banner').get();
    if (bannerDoc.exists) {
...
async function salvarConfigLoja() {
  ...
  try {
    await refConfigFilial('loja').set({
      nome,
      unidade,
      filialId: idFilialAtual(),
      lastUpdate: firebase.firestore.FieldValue.serverTimestamp()
...
async function salvarBannerConfig() {
  ...
  try {
    await refConfigFilial('banner').set({ url, filialId: idFilialAtual(), lastUpdate: firebase.firestore.FieldValue.serverTimestamp() });
...
async function loadPersonalizacao() {
  try {
    const doc = await refConfigFilial('personalizacao').get();
...
async function salvarPersonalizacao() {
  ...
  try {
    await refConfigFilial('personalizacao').set(comFilial(data));
    aplicarCoresTema({
...
async function salvarExibicaoCardapio() {
  ...
  try {
    await refConfigFilial('personalizacao').set(
      { mostrarEstoque, filialId: idFilialAtual(), lastUpdate: firebase.firestore.FieldValue.serverTimestamp() },
      { merge: true }
    );
...
function loadExibicaoCardapio() {
  ...
  if (!toggle) return;
  refConfigFilial('personalizacao').get().then(doc => {
...
async function salvarHorarioFuncionamento() {
  ...
  try {
    await refConfigFilial('personalizacao').set(comFilial(data), { merge: true });
...
async function salvarMetodosPagamento() {
  await refConfigFilial('pagamentos').set({
...
async function salvarClassificacoes() {
  await refConfigFilial('classificacoes').set({
...
async function conciliarVendasNoCaixa() {
  ...
  try {
    ...
    for (const venda of db.vendas) {
      ...
      if (normalizarStatus(venda.status) === 'cancelados') continue;
      if (venda.origem === 'cliente' && venda.estoqueBaixado !== true) continue;
...
async function updateOrderStatus(id, newStatus) {
  try {
    const vendaRef = dbFirestore.collection('vendas').doc(id);
    await dbFirestore.runTransaction(async transaction => {
      const vendaSnap = await transaction.get(vendaRef);
      if (!vendaSnap.exists) throw new Error('Pedido não encontrado.');
      const venda = vendaSnap.data();
      if (filialDoRegistro(venda) !== idFilialAtual()) throw new Error('Pedido pertence a outra filial.');

      const deveBaixar = venda.origem === 'cliente' && !venda.estoqueBaixado && newStatus !== 'Cancelado';
      const deveEstornar = venda.origem === 'cliente' && venda.estoqueBaixado && newStatus === 'Cancelado';
      const alteracoes = { status: newStatus };

      if (deveBaixar || deveEstornar) {
        const quantidades = new Map();
        (venda.itens || []).forEach(item => {
          const qtd = Number(item.qtdCarrinho || 0);
          if (!item.id || !Number.isInteger(qtd) || qtd <= 0) throw new Error('Pedido contém item inválido.');
          quantidades.set(item.id, (quantidades.get(item.id) || 0) + qtd);
        });
        if (!quantidades.size) throw new Error('Pedido sem itens válidos.');

        const produtos = [];
        for (const [produtoId, qtd] of quantidades.entries()) {
          const ref = dbFirestore.collection('estoque').doc(produtoId);
          const snap = await transaction.get(ref);
          if (!snap.exists) throw new Error('Um produto do pedido não existe mais.');
          const produto = snap.data();
          if (filialDoRegistro(produto) !== idFilialAtual()) throw new Error('Produto pertence a outra filial.');
          const estoqueAtual = Number(produto.qtd || 0);
          if (deveBaixar && estoqueAtual < qtd) throw new Error(`Estoque insuficiente para ${produto.nome || produtoId}.`);
          produtos.push({ ref, produtoId, produto, qtd, estoqueAtual });
        }

        produtos.forEach(({ ref, qtd, estoqueAtual }) => {
          transaction.update(ref, { qtd: deveBaixar ? estoqueAtual - qtd : estoqueAtual + qtd });
        });

        if (deveBaixar) {
          const itensConfirmados = produtos.map(({ produtoId, produto, qtd }) => ({
            id: produtoId,
          ...
          alteracoes.itens = itensConfirmados;
          alteracoes.total = itensConfirmados.reduce((soma, item) => soma + item.preco * item.qtdCarrinho, 0);
          alteracoes.estoqueBaixado = true;
          alteracoes.aceitoEm = new Date().toISOString();
          alteracoes.aceitoPor = auth.currentUser?.uid || null;
        } else {
          alteracoes.estoqueBaixado = false;
          alteracoes.estornadoEm = new Date().toISOString();
        }
      }

      transaction.update(vendaRef, alteracoes);
    });
  ...
  const conteudo = `
    <html>
      ...
      <body>
        ...
        <div class="info">
          <strong>Cliente:</strong> ${venda.clienteNome}<br>
          <strong>Tel:</strong> ${venda.clienteTel || ''}<br>
          <strong>Endereço:</strong> ${venda.clienteEndereco || ''}<br>
...
function aplicarRestricoesNav() {
  ...
  const abaBalanco = document.getElementById('aba-rel-balanco-filiais');
  if (abaBalanco) abaBalanco.classList.toggle('hidden', !ehAdministrador());
}
...
function ehAdministrador() {
  ...
  const registro = (db.usuarios || []).find(u => u.id === user.uid) || perfilOperadorAtual;
  return !!(registro && registro.role === 'admin');
}

function normalizarIdFilial(valor) {
  return String(valor || '')
    .normalize('NFD').replace(/[\u0300-\u036f]/g, '')
    .toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '');
}

function renderizarGestaoFiliais() {
  const card = document.getElementById('config-filiais-card');
  const lista = document.getElementById('lista-filiais');
  if (card) card.classList.toggle('hidden', !ehAdministrador());
  if (!lista || !ehAdministrador()) return;
  const itens = catalogoFiliais();
  lista.innerHTML = itens.map(f => `
    <div class="border border-neutral-700 rounded-lg p-4 flex items-center justify-between gap-3">
      <div class="min-w-0">
        <p class="font-bold text-white truncate">${escapeDashboard(f.nome || f.id)}</p>
        <p class="text-xs text-gray-500">${escapeDashboard(f.codigo || f.id)}${f.cnpj ? ' · ' + escapeDashboard(f.cnpj) : ''}</p>
      </div>
      <span class="text-xs px-2 py-1 rounded ${f.ativa === false ? 'bg-red-500/15 text-red-400' : 'bg-emerald-500/15 text-emerald-400'}">${f.ativa === false ? 'INATIVA' : 'ATIVA'}</span>
    </div>`).join('');
}

async function salvarFilialBasica() {
  if (!ehAdministrador()) return showToast('Somente administradores podem cadastrar filiais.', 'error');
  ...
  const cnpj = (document.getElementById('filial-cnpj')?.value || '').trim();
  const id = normalizarIdFilial(codigo || nome);
  if (!id || !nome) return showToast('Informe o código e o nome da filial.', 'error');
  try {
    await dbFirestore.collection('filiais').doc(id).set({
      codigo: codigo || id.toUpperCase(),
      nome,
      cnpj,
      ativa: true,
      atualizadoEm: firebase.firestore.FieldValue.serverTimestamp()
    }, { merge: true });
    document.getElementById('filial-codigo').value = '';
    document.getElementById('filial-nome').value = '';
    document.getElementById('filial-cnpj').value = '';
    registrarLog('incluir', 'filial', id, `Filial "${nome}" cadastrada`, { codigo, cnpj });
    showToast(`Filial ${nome} salva com sucesso.`, 'success');
  } catch (error) {
    console.error('Erro ao salvar filial:', error);
    showToast('Não foi possível salvar a filial. Publique primeiro as novas regras do Firestore.', 'error');
  }
}

function opcoesFiliaisFuncionario(usuario) {
  const selecionadas = Array.isArray(usuario.filiais) && usuario.filiais.length
    ? usuario.filiais
    : [FILIAL_PADRAO_ID];
  const itens = catalogoFiliais();
  return itens.map(f => `<option value="${escapeDashboard(f.id)}" ${selecionadas.includes(f.id) ? 'selected' : ''}>${escapeDashboard(f.nome || f.id)}</option>`).join('');
}
...
function atualizarVisibilidadeCardsPerfil() {
  ...
  const cardFiliais = document.getElementById('config-filiais-card');
  if (cardFuncionarios) cardFuncionarios.classList.toggle('hidden', !isAdmin);
  // Elevação de papel não pode ser validada com segurança apenas no cliente.
  if (cardMaster) cardMaster.classList.add('hidden');
  if (cardFiliais) cardFiliais.classList.toggle('hidden', !isAdmin);
}
  ...
  lista.innerHTML = db.usuarios.map(u => {
    ...
    return `
      <div class="border ${bloqueado ? 'border-red-500/60' : 'border-gray-700'} rounded-lg p-3 space-y-3">
        ...
        <div>
          <label class="text-xs text-gray-400 block mb-1">Filiais permitidas</label>
          <select id="func-filiais-${u.id}" multiple class="input-field min-h-24" ${ehAdmin ? 'disabled' : ''}>
            ${opcoesFiliaisFuncionario(u)}
          </select>
          ${ehAdmin ? '<p class="text-[11px] text-gray-500 mt-1">Administradores acessam todas as filiais.</p>' : ''}
        </div>
...
async function salvarFuncionario(uid) {
  ...
  const selectFiliais = document.getElementById(`func-filiais-${uid}`);
  const filiais = selectFiliais
    ? Array.from(selectFiliais.selectedOptions).map(option => option.value)
    : (atual?.filiais || [FILIAL_PADRAO_ID]);
  ...
  try {
    await dbFirestore.collection('usuarios').doc(uid).update({
      ...
      filiais: role === 'admin' ? [] : (filiais.length ? filiais : [FILIAL_PADRAO_ID]),
...
const MODULOS_SISTEMA = [
  ...
  { id: 'balanco-filiais', nome: 'Balanço de Filiais', icon: 'fa-building-columns' },
...
function podeAcessarModulo(moduloId) {
  ...
  if (!user) return false;
  const registro = (db.usuarios || []).find(u => u.id === user.uid) || perfilOperadorAtual;
  if (moduloId === 'balanco-filiais' && registro?.role !== 'admin') return false;
...
function normalizarStatus(status = '') {
  const value = status.toLowerCase().normalize('NFD').replace(/[u0300-u036f]/g, '');
...
const time = date.getTime() ? date.toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' }) : '--:--';
    ...
    const delivery = venda.clienteEndereco || venda.endereco ? 'Delivery' : 'Retirada';
    ...
    return `<article class="order-card" onclick="abrirModalDetalhes('${venda.id}')">
      <div><h3 class="order-number">#${escapeDashboard(numeroExibicao(venda))}</h3>
      <p class="order-customer">${escapeDashboard(venda.clienteNome || 'Cliente balcão')}</p>
      <div class="order-meta"><span><i class="fas ${delivery === 'Delivery' ? 'fa-truck-fast' : 'fa-store'}"></i>${delivery}</span><span><i class="far fa-clock"></i>${quando}</span></div>
      <div class="order-product-images">${thumbs}${more}</div></div>
<div class="order-right"><span class="status-pill ${status.className}">${status.label}</span><strong class="order-total">${formatarMoeda(venda.total || 0)}</strong></div>
    </article>`;
  }).join('');
...
function mostrarAbaRelatorio(aba) {
  ...
  document.getElementById('rel-balanco-filiais').classList.toggle('hidden', aba !== 'balanco-filiais');
  document.getElementById('aba-rel-vendas').classList.toggle('active', aba === 'vendas');
  ...
  document.getElementById('aba-rel-balanco-filiais').classList.toggle('active', aba === 'balanco-filiais');
  if (aba === 'balanco-filiais') renderizarRelatorioBalancoFiliais();
}
...
/* ================= BALANÇO DE FILIAIS ================= */
function listaConsolidada(nome, fallback = []) {
  return Array.isArray(dadosConsolidadosFiliais[nome])
    ? dadosConsolidadosFiliais[nome]
    : fallback;
}

function dataValidaRegistro(registro, campos) {
  for (const campo of campos) {
    const valor = registro && registro[campo];
    if (!valor) continue;
...
function intervaloBalanco(prefixo) {
  const inicioValor = document.getElementById(`${prefixo}-inicio`)?.value || '';
  const fimValor = document.getElementById(`${prefixo}-fim`)?.value || '';
  const inicio = inicioValor ? new Date(`${inicioValor}T00:00:00`) : null;
  const fim = fimValor ? new Date(`${fimValor}T23:59:59.999`) : null;
  return { inicio, fim };
}

function dentroDoPeriodo(registro, campos, inicio, fim) {
  if (!inicio && !fim) return true;
  const data = dataValidaRegistro(registro, campos);
...
function coletarBalancoFiliais(prefixo = 'balanco') {
  const { inicio, fim } = intervaloBalanco(prefixo);
  const vendas = listaConsolidada('vendas', db.vendas);
  const movimentos = listaConsolidada('movimentacoes_caixa', movimentacoesCaixa);
  const contas = listaConsolidada('contas_pagar', contasPagarLista);
  const entradas = listaConsolidada('entradas', entradasLista);
  const permitidas = new Set(filiaisPermitidasDoUsuario());
  const idsEncontrados = new Set([
    ...vendas.map(filialDoRegistro),
    ...movimentos.map(filialDoRegistro),
    ...contas.map(filialDoRegistro),
    ...entradas.map(filialDoRegistro)
  ]);
  const baseFiliais = catalogoFiliais()
    .filter(f => permitidas.has(f.id));
  idsEncontrados.forEach(id => {
    if (permitidas.has(id) && !baseFiliais.some(f => f.id === id)) {
      baseFiliais.push({ id, nome: id === FILIAL_PADRAO_ID ? 'Filial Matriz' : id });
    }
  });

  const linhas = baseFiliais.map(filial => {
    const vendasFilial = vendas.filter(v => filialDoRegistro(v) === filial.id)
      .filter(v => dentroDoPeriodo(v, ['dataIso', 'submittedAt', 'data'], inicio, fim))
      .filter(v => String(v.status || '').toLowerCase() !== 'cancelado')
      .filter(v => v.origem !== 'cliente' || v.estoqueBaixado === true);
    const despesasFilial = movimentos.filter(m => filialDoRegistro(m) === filial.id)
      .filter(m => m.tipo === 'DESPESA')
      .filter(m => dentroDoPeriodo(m, ['data_hora'], inicio, fim));
    const contasFilial = contas.filter(c => filialDoRegistro(c) === filial.id)
      .filter(c => c.status === 'PENDENTE')
      .filter(c => dentroDoPeriodo(c, ['dataVencimento', 'dataIso', 'criadoEm'], inicio, fim));
    const entradasFilial = entradas.filter(e => filialDoRegistro(e) === filial.id)
      .filter(e => e.status === 'CONFIRMADA')
      .filter(e => dentroDoPeriodo(e, ['dataEntrada', 'dataEmissao', 'dataIso'], inicio, fim));
    const faturamento = vendasFilial.reduce((s, v) => s + Number(v.total || 0), 0);
    const despesas = despesasFilial.reduce((s, m) => s + Number(m.valor || 0), 0);
    const pendente = contasFilial.reduce((s, c) => s + Math.max(0, Number(c.valor || 0) - Number(c.valorPago || 0)), 0);
    const totalEntradas = entradasFilial.reduce((s, e) => s + Number(e.total || 0), 0);
    return {
      filial,
      pedidos: vendasFilial.length,
      faturamento,
      despesas,
      saldo: faturamento - despesas,
      pendente,
      totalEntradas,
      notas: entradasFilial.filter(e => e.nota || e.chave).length
    };
  });
  ...
  return {
    ...
    entradas: linhas.reduce((s, l) => s + l.totalEntradas, 0)
  };
}

function linhasHtmlBalanco(rel) {
  return rel.linhas.length ? rel.linhas.map(l => `
    <tr class="border-b border-neutral-700/70 hover:bg-neutral-700/30">
      <td class="py-3 pr-4 font-semibold text-white">${escapeDashboard(l.filial.nome || l.filial.id)}</td>
      <td class="py-3 pr-4 text-right">${l.pedidos}</td>
      <td class="py-3 pr-4 text-right text-emerald-400">${formatarMoeda(l.faturamento)}</td>
      <td class="py-3 pr-4 text-right text-red-400">${formatarMoeda(l.despesas)}</td>
      <td class="py-3 pr-4 text-right font-bold ${l.saldo < 0 ? 'text-red-400' : 'text-amber-400'}">${formatarMoeda(l.saldo)}</td>
      <td class="py-3 pr-4 text-right text-orange-400">${formatarMoeda(l.pendente)}</td>
      <td class="py-3 text-right text-sky-400">${formatarMoeda(l.totalEntradas)} <span class="text-xs text-gray-500">(${l.notas} NF)</span></td>
    </tr>`).join('') : '<tr><td colspan="7" class="py-8 text-center text-gray-500">Nenhuma filial disponível.</td></tr>';
}

function renderizarBalancoFiliais() {
  const corpo = document.getElementById('balanco-filiais-corpo');
  if (!corpo) return;
  const rel = coletarBalancoFiliais('balanco');
  corpo.innerHTML = linhasHtmlBalanco(rel);
  const campos = {
    'balanco-total-faturamento': rel.faturamento,
    'balanco-total-despesas': rel.despesas,
    'balanco-total-saldo': rel.saldo,
    'balanco-total-pendente': rel.pendente,
    'balanco-total-entradas': rel.entradas
  };
...
function renderizarRelatorioBalancoFiliais() {
  const corpo = document.getElementById('rel-balanco-corpo');
  if (!corpo) return;
  const rel = coletarBalancoFiliais('rel-balanco');
  corpo.innerHTML = linhasHtmlBalanco(rel);
  const info = document.getElementById('rel-balanco-info');
  if (info) info.textContent = `${rel.linhas.length} filial(is) · Faturamento ${formatarMoeda(rel.faturamento)} · Despesas ${formatarMoeda(rel.despesas)} · Saldo ${formatarMoeda(rel.saldo)}`;
}

function exportarBalancoFiliaisPDF(usarRelatorio = false) {
  const prefixo = usarRelatorio ? 'rel-balanco' : 'balanco';
  const rel = coletarBalancoFiliais(prefixo);
  const thead = '<thead><tr><th>Filial</th><th class="r">Pedidos</th><th class="r">Faturamento</th><th class="r">Despesas</th><th class="r">Saldo</th><th class="r">A pagar</th><th class="r">Entradas/NF</th></tr></thead>';
  const corpo = rel.linhas.map(l => `<tr><td>${escapeDashboard(l.filial.nome || l.filial.id)}</td><td class="r">${l.pedidos}</td><td class="r">${formatarMoeda(l.faturamento)}</td><td class="r">${formatarMoeda(l.despesas)}</td><td class="r">${formatarMoeda(l.saldo)}</td><td class="r">${formatarMoeda(l.pendente)}</td><td class="r">${formatarMoeda(l.totalEntradas)} (${l.notas} NF)</td></tr>`).join('');
  const rodape = `<tfoot><tr><td colspan="2">Total consolidado</td><td class="r">${formatarMoeda(rel.faturamento)}</td><td class="r">${formatarMoeda(rel.despesas)}</td><td class="r">${formatarMoeda(rel.saldo)}</td><td class="r">${formatarMoeda(rel.pendente)}</td><td class="r">${formatarMoeda(rel.entradas)}</td></tr></tfoot>`;
  const periodo = `${rel.inicio ? rel.inicio.toLocaleDateString('pt-BR') : 'início'} a ${rel.fim ? rel.fim.toLocaleDateString('pt-BR') : 'hoje'}`;
  abrirImpressaoRelatorio('Balanço de Filiais', `Período: ${periodo} · Gerado em ${new Date().toLocaleString('pt-BR')}`, `<table>${thead}<tbody>${corpo}</tbody>${rodape}</table>`);
}
...
async function proximoNumeroContaPagar() {
  const ref = refConfigFilial('contador_conta_pagar');
  const resultado = await dbFirestore.runTransaction(async (tx) => {
    const snap = await tx.get(ref);
    const atual = snap.exists ? Number(snap.data().numero || 0) : 0;
    const proximo = atual + 1;
...
async function cancelarContaPagar(id) {
  ...
  if (!confirm(`Cancelar a conta #${c.numero || id} — ${formatarMoeda(c.valor)}?`)) return;
  try {
...
async function proximoNumeroEntrada() {
  const ref = refConfigFilial('contador_entrada');
  const resultado = await dbFirestore.runTransaction(async (tx) => {
    const snap = await tx.get(ref);
    const atual = snap.exists ? Number(snap.data().numero || 0) : 0;
    const proximo = atual + 1;
...
async function confirmarEntrada() {
  ...
  if (!confirm(reconfirmandoEstornada
    ? 'Confirmar NOVAMENTE esta entrada estornada? O estoque dos itens será atualizado.'
    : 'Confirmar a entrada? O estoque dos itens será atualizado e não poderá ser desfeito diretamente.')) return;
...
async function estornarEntrada(id) {
  ...
  const avisoConta = contaVinculada
    ? 'nnATENÇÃO: há uma conta a pagar pendente vinculada a esta entrada. Ela será CANCELADA junto com o estorno.'
    : '';
  if (!confirm(`Estornar a entrada #${e.numero || id}? O estoque será restaurado (movimentação inversa).${avisoConta}`)) return;
...
async function excluirEntrada(id) {
  ...
  if (!confirm(`Excluir ${rotulo} ${e.numero ? '#' + e.numero : ''}?`)) return;
  try {
...
async function confirmarEntradaExistente(id) {
  ...
  const msg = reconfirmando
    ? `Confirmar novamente a entrada #${e.numero || id}? O estoque dos itens será atualizado novamente.`
    : 'Confirmar a entrada? O estoque dos itens será atualizado.';
...
async function addProdutoCadastro() {
  ...
  try {
    if (id) {
      ...
      const docRef = await dbFirestore.collection('estoque').add({ filialId: idFilialAtual(), nome, descricao, imagem, marca, grupo, subgrupo, codigoBarras, preco, qtd, codigo: proximoCodigoProduto(), bloqueado: false, lastUpdate: firebase.firestore.FieldValue.serverTimestamp() });
...
async function addClienteCadastro() {
  ...
  try {
    if (editId) {
      await dbFirestore.collection('clientes').doc(editId).set({ filialId: idFilialAtual(), nome, tel, email, endereco, numero, cep }, { merge: true });
      ...
      const docRef = await dbFirestore.collection('clientes').add({ filialId: idFilialAtual(), nome, tel, email, endereco, numero, cep, codigo: proximoCodigoCliente(), bloqueado: false, since: firebase.firestore.FieldValue.serverTimestamp() });
...
async function salvarFornecedorCadastro() {
  ...
  try {
    if (editId) {
      await dbFirestore.collection('fornecedores').doc(editId).set({ filialId: idFilialAtual(), nome, documento, contato, email, endereco, numero, observacao, atualizadoEm: firebase.firestore.FieldValue.serverTimestamp() }, { merge: true });
      registrarLog('editar', 'fornecedor', editId, `Fornecedor "${nome}" atualizado (Cadastros)`, { nome, documento });
      showToast('Fornecedor atualizado com sucesso!', "success");
    } else {
      const ref = await dbFirestore.collection('fornecedores').add({ filialId: idFilialAtual(), nome, documento, contato, email, endereco, numero, observacao, codigo: proximoCodigoFornecedor(), criadoEm: firebase.firestore.FieldValue.serverTimestamp() });
  ...
  container.innerHTML = mesasLista.map(mesa => {
    ...
    return `
      <div onclick="abrirMesa('${mesa.id}')" class="relative group cursor-pointer border rounded-xl p-4 flex flex-col items-center justify-center transition-all hover:scale-105 ${bgClass}">
        <h3 class="text-3xl font-bold text-white mb-2">${mesa.numero}</h3>
        <span class="text-xs font-semibold uppercase tracking-wider ${textClass}">${statusText}</span>
        ${isOcupada ? `<span class="mt-2 text-sm font-bold text-amber-500">${formatarMoeda(total)}</span>` : ''}
      </div>
    `;
  }).join('');
}
...
async function salvarConfigMesas() {
  ...
  try {
    ...
    for (let i = 1; i <= qtd; i++) {
      ...
      const mesaId = idFilialAtual() === FILIAL_PADRAO_ID ? `mesa_${numeroStr}` : `${idFilialAtual()}__mesa_${numeroStr}`;
      const ref = dbFirestore.collection('mesas').doc(mesaId);
      const existe = mesasLista.find(m => m.id === mesaId);
      if (!existe) {
        batch.set(ref, {
          filialId: idFilialAtual(),
          numero: numeroStr,
...
function abrirMesa(mesaId) {
  ...
  mesaAtualId = mesaId;
  comandaItens = (mesa.itens || []).map(i => ({ ...i })); // Cópia dos itens
  
  document.getElementById('comanda-titulo').textContent = `Mesa ${mesa.numero}`;
  document.getElementById('comanda-subtitulo').textContent = mesa.status === 'ocupada' ? `Em andamento` : `Livre`;
  document.getElementById('comanda-busca').value = '';
...
function renderizarProdutosComanda() {
  ...
  container.innerHTML = produtos.map(p => `
    <div onclick="adicionarItemComanda('${p.id}')" class="flex justify-between items-center p-2 hover:bg-gray-800 rounded cursor-pointer border border-transparent hover:border-gray-700 transition-colors">
      <div class="flex-1 min-w-0">
        <p class="text-white text-sm font-semibold truncate">${p.nome}</p>
        <p class="text-xs text-gray-400">Estoque: <span class="${p.qtd <= 0 ? 'text-red-400' : 'text-green-400'}">${p.qtd}</span></p>
      </div>
      <span class="text-amber-500 font-bold ml-2">${formatarMoeda(p.preco)}</span>
    </div>
  `).join('');
}
...
function adicionarItemComanda(produtoId) {
  ...
  const idx = comandaItens.findIndex(i => i.id === produtoId);
  if (idx >= 0) {
    if (comandaItens[idx].qtd < produto.qtd) {
      comandaItens[idx].qtd += 1;
    } else {
      return showToast('Estoque insuficiente.', 'warning');
    }
  } else {
    if (produto.qtd <= 0) return showToast('Produto sem estoque.', 'warning');
    comandaItens.push({ id: produto.id, nome: produto.nome, preco: produto.preco, qtd: 1 });
  }
  
  salvarComandaItens();
}

function atualizarQtdComanda(index, delta) {
  if (!mesaAtualId || index < 0 || index >= comandaItens.length) return;
  const item = comandaItens[index];
  const produto = db.estoque.find(p => p.id === item.id);
  const estoqueDisponivel = produto ? produto.qtd : 0;
  
  const novaQtd = item.qtd + delta;
  if (novaQtd <= 0) {
    comandaItens.splice(index, 1);
  } else if (novaQtd > estoqueDisponivel) {
    return showToast('Estoque insuficiente.', 'warning');
  } else {
    comandaItens[index].qtd = novaQtd;
  }
...
async function salvarComandaItens() {
  if (!mesaAtualId) return;
  const total = comandaItens.reduce((s, i) => s + (i.preco * i.qtd), 0);
  const status = total > 0 ? 'ocupada' : 'disponivel';
  ...
  try {
    ...
    showToast('Erro ao atualizar a comanda.', 'error');
  }
}
...
function renderizarComandaItens() {
  ...
  const btnFinalizar = document.querySelector('button[onclick="finalizarComandaAtual()"]');
  if (!container || !totalEl) return;
  
  const total = comandaItens.reduce((s, i) => s + (i.preco * i.qtd), 0);
  
  if (btnFinalizar) {
    btnFinalizar.disabled = comandaItens.length === 0;
  }
  
  if (comandaItens.length === 0) {
    container.innerHTML = '<p class="text-gray-500 italic text-center py-4">Nenhum item na comanda.</p>';
    totalEl.textContent = formatarMoeda(0);
    return;
  }
  
  container.innerHTML = comandaItens.map((item, idx) => {
    const sub = item.preco * item.qtd;
    return `
      <div class="flex justify-between items-center bg-gray-800 p-3 rounded-lg border border-gray-700 mb-2">
        <div class="flex-1 min-w-0 pr-2">
          <p class="font-semibold text-white text-sm truncate">${item.nome}</p>
          <p class="text-xs text-gray-400">${item.qtd} x ${formatarMoeda(item.preco)}</p>
        </div>
        <div class="flex items-center gap-2">
          <button onclick="atualizarQtdComanda(${idx}, -1)" class="text-red-500 hover:text-red-400 p-1"><i class="fa fa-minus-circle"></i></button>
          <span class="text-white font-bold text-sm w-6 text-center">${item.qtd}</span>
          <button onclick="atualizarQtdComanda(${idx}, 1)" class="text-green-500 hover:text-green-400 p-1"><i class="fa fa-plus-circle"></i></button>
        </div>
        <span class="text-amber-500 font-bold text-sm w-20 text-right">${formatarMoeda(sub)}</span>
        <button onclick="removerItemComanda(${idx})" class="text-gray-500 hover:text-red-400 p-1"><i class="fa fa-trash text-xs"></i></button>
      </div>`;
  }).join('');
...
function fecharMesaAtual() {
  ...
  const textoBtn = comandaItens.length === 0 ? 'Liberar Mesa' : 'Fechar e gerar pedido';
  
  abrirModalConfirmacao({
    titulo: `Fechar Mesa ${mesa.numero}?`,
    mensagem: comandaItens.length === 0 ? 'Mesa vazia. Deseja apenas liberar a mesa?' : `Comanda ${mesa.numeroPedido || '---'}`,
    detalhe: comandaItens.length === 0 ? '' : `Total: ${formatarMoeda(total)}`,
    textoBtn: textoBtn,
    corBtn: 'bg-red-600 hover:bg-red-500 text-white',
    icone: 'fa-lock',
    iconeCor: 'bg-red-500/20',
    onConfirm: async () => {
      try {
        ...
        await batch.commit();
        await registrarMovimentacaoVendaCaixa(vendaDoc.id, numeroPedido, 'Mesa', total, venda);
        await dbFirestore.collection('mesas').doc(mesaAtualId).update({
          status: 'disponivel',
          itens: [],
          total: 0,
        ...
        registrarLog('venda', 'venda', vendaDoc.id, `Mesa ${mesa.numero} fechada — Pedido #${numeroPedido} — ${formatarMoeda(total)}`, { numeroPedido, total, mesaId: mesaAtualId });
        showToast(`Mesa ${mesa.numero} fechada — Pedido #${numeroPedido} gerado!`, 'success');
        fecharComandaPanel();
      } catch (err) {
...
async function finalizarComandaAtual() {
  if (!mesaAtualId) return;
  const mesa = mesasLista.find(m => m.id === mesaAtualId);
  if (!mesa || comandaItens.length === 0) return showToast('Adicione produtos antes de finalizar.', 'error');
  const total = comandaItens.reduce((s, i) => s + (i.preco * i.qtd), 0);
  ...
  try {
    ...
    registrarLog('venda', 'venda', vendaDoc.id, `Comanda #${numeroPedido} finalizada — Mesa ${mesa.numero} — ${formatarMoeda(total)}`, { numeroPedido, total, mesaId: mesaAtualId });
    showToast(`Comanda #${numeroPedido} finalizada!`, 'success');
  } catch (err) {
  ...
  let html = `
    <html>
      ...
      <body>
        ...
        <p style="text-align:center; margin:0;">${new Date().toLocaleDateString('pt-BR')} ${new Date().toLocaleTimeString('pt-BR')}</p>
        <hr>
  `;
...
window.atualizarQtdComanda = atualizarQtdComanda;
window.removerItemComanda = removerItemComanda;
window.salvarComandaItens = salvarComandaItens;
window.renderizarComandaItens = renderizarComandaItens;
window.fecharMesaAtual = fecharMesaAtual;
window.finalizarComandaAtual = finalizarComandaAtual;
window.imprimirComandaAtual = imprimirComandaAtual;
