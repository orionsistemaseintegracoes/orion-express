...
- `filiais/{filialId}` cadastra código, nome, CNPJ e situação da unidade.
- As coleções operacionais usam `filialId` para separar os dados.
- `usuarios/{uid}.filiais` limita as unidades de funcionários; administradores acessam todas.
- Configurações da Matriz preservam os IDs legados. Outras filiais usam IDs no formato `{filialId}__{config}`.
- Documentos legados sem `filialId` pertencem à filial `matriz` durante a migração.

## Conexão

Configuração em `script.js` (projeto **`orion-express-pdv`**):

- `apiKey`, `authDomain`, `projectId`, `storageBucket`, `messagingSenderId`, `appId`, `measurementId`
- ⚠️ Comentário no código: substituir pelas chaves do novo projeto **"orion-express"** no Firebase Console.

> ⚠️ **Segurança**: a `apiKey` fica exposta no cliente (comum em Firebase web). A proteção real vem de `firestore.multifilial.rules`, apontado por `firebase.json`.
...
| `filialId` | string | Unidade dona do pedido |
| `origem` | string | `'admin'` ou `'cliente'` |
| `estoqueBaixado` | boolean | Confirma que o pedido já foi validado pelo operador e baixado no estoque |
...
> **VENDA automática**: vendas do PDV admin (`finalizarVenda`) entram no caixa aberto. Pedidos do cliente só são conciliados depois que o operador os aceita; nesse aceite, preço e total são recalculados pelo catálogo da filial e o estoque é baixado em transação. Pedidos pendentes ou cancelados não entram no caixa nem no balanço.
...
> **Regras de Firestore locais** (`firestore.multifilial.rules`, ainda não publicadas):
...
- **Sincronização do contador** (garante o contador ≥ maior `numeroPedido`, **reformata os existentes para `###`** e numera os pedidos antigos **sem número**, do mais antigo ao mais recente):
  - Admin: `migrarNumeroPedido()` no login de administradores — reformata formatos legados (ex.: `000027` → `027`).
  - Cliente: apenas incrementa atomicamente o contador da filial; não executa migração global.
