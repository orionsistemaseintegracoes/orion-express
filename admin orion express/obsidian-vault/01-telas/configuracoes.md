...
#### Cartão 3 — Mesas (Comanda)
- Campo **Quantidade de mesas ativas** (`#mesas-qtd`).
- Botão **SALVAR QUANTIDADE DE MESAS** → `salvarConfigMesas()`.
- Mensagem `#mesas-config-status`.
- Gerencia a criação e exclusão segura das mesas em tempo real no Firestore (`mesas/mesa_XX`).
