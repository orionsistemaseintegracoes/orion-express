...
> PDV + Gestão de Pedidos online da **Orion Express**. Aplicação web de página única (SPA), tema escuro, alimentada em tempo real via **Supabase** através da camada de compatibilidade **`supabase-compat.js`** (emulando a API Firebase Auth + Firestore).

- 📁 Pasta do projeto: `orion-express-admin`
- 🧩 Arquivos principais: `index.html`, `style.css`, `script.js`, `supabase-compat.js`, `AGENTS.md`, `seed-firestore.js`, `check-vendas.js`, `orion-logo.png`, `design-qa.md`
