...
```
Browser (index.html + script.js + supabase-compat.js)
        │
        ▼ (Supabase JS SDK)
┌─────────────────────────────────────┐
...
- **Autenticação**: Supabase Auth (e-mail/senha) exposto via camada mockada de `firebase.auth()`.
- **Dados**: lidos e escritos via `supabase-compat.js` (emulando `firebase.firestore()`). O app mantém um cache local (`db`) sincronizado em tempo real por listeners `onSnapshot` conectados a canais do Supabase.
